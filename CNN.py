...
class TradingSignalCNN:
    def __init__(self, model_path='cnn_train_50.h5'):
    ...
    def add_signal_trace(self, fig, df):
        fig.add_trace(go.Scatter(
            ...
            mode='markers', 
            name='Buy Signal', 
            marker=dict(color='green', symbol='triangle-up', size=16, opacity=0.8),
        ))
        fig.add_trace(go.Scatter(
            x=df.index, 
            y=df['Sell'], 
            mode='markers', 
            name='Sell Signal', 
            marker=dict(color='red', symbol='triangle-down', size=16, opacity=0.8),
        ))
        return fig